...
export default function JournalListing({
  ...
  return (
    ...
    >
      {/* Page header */}
      <header className="grid grid-cols-[1fr_auto] items-end gap-8 border-b border-foreground/10 pb-10 pt-4">
        <div className="flex flex-col gap-5">
          {kicker && (
            <SlideIn delay={0.1} as="span">
              <span className="inline-flex items-center gap-2.5 font-heading text-[11px] uppercase tracking-[0.14em] text-foreground/55 before:block before:size-1.5 before:rounded-full before:bg-foreground/50">
                {kicker}
              </span>
            </SlideIn>
          )}
        ...
        <FadeIn delay={0.35} className="self-start pt-2 text-right">
      ...
      {/* Interactive listing */}
      {initialData && <JournalListingClient initialData={initialData} />}
    </ComponentLayout>
  )
}
