...
/**
 * The single render registry. Typed as a mapped type over `ComponentTypeName`, so
 * every component type MUST have an entry — omit one and this file fails to compile,
 * instead of the component silently rendering as `null` at runtime. Each entry is
 * typed to its own component's props (no `any`), so the two non-uniform cases —
 * `HomePageHero` (needs `scrollTargetId`) and `Form` → `GetInTouch` (name mismatch) —
 * are just ordinary entries.
 */
...
export const organismComponents: OrganismComponents = {
 ...
 * the runtime `_type` to its entry — this is the one escape hatch any value-keyed
 * dispatch needs, and it is an assertion, not `any`.
 */
export type OrganismRenderer = (
